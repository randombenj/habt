...
from habt.importer.helper import Contents, Descriptions
...
class Source:
    ...
    def packages(self):
        ...
        for entry in self._source_list.entries:
            ...
            sources_content = self.__get_and_decode(
                entry.build_uri(entry.sources_file["Sources"]["name"])
            )
            descriptions_content = self.__get_and_decode(
                entry.build_uri(entry.descriptions_file["Descriptions"]["name"])
            )
            source_list_entry = {
                "Entry": entry,
                "Sources": self.__parser["Sources"](sources_content)
                if sources_content
                else [],
                "Descriptions": self.__parser["Descriptions"](descriptions_content)
            ...
            for architecture in entry.architectures:
                ...
                architecture_entry = {"Architecture": architecture}

                for f in files:
                    architecture_entry.update(
                        {
                            f: self.__parser[f](
                                self.__get_and_decode(entry.build_uri(files[f]["name"]))
                            )
                        }
                    )
...
class SourceList:
    def __init__(self, sources_list_file):
        ...
        with open(sources_list_file) as sources_list:
            ...
            self._raw_sources_list = [
                s.strip()
                for s in sources_list.readlines()
...
class SourceListEntry:
    def __init__(self, source):
        ...
        if response.ok:
            ...
            self._files = [
                f
                for f in release["SHA256"]
                if (
                    # ignore udeb and debian-installer files
                    "-udeb" not in f["name"]
                    and "debian-installer" not in f["name"]
                    and
                    # use only the gzip compressed files
                    (".gz" in f["name"] or "Translation-en.bz2" in f["name"])
                    and any(
                        [
                    ...
                    and any(
                        [
                            ...
                            if (
                                a in f["name"]
                                or "Sources" in f["name"]
                                or "Translation-en" in f["name"]
                            )
                        ]
                    )
                )
    ...
    def build_uri(self, requested_file):
        ...
        return "{0}/dists/{1}/{2}".format(
            self._archive, self._distribution, requested_file
        )
    ...
    def sources_file(self):
        ...
        """
        return {"Sources": next(f for f in self._files if "Sources" in f["name"])}
    ...
    def descriptions_file(self):
        ...
        return {
            "Descriptions": next(
                (f for f in self._files if "Translation-en" in f["name"]),
                {"name": ""},  # default value
            )
        }
    ...
    def get_files(self, architecture):
        ...
        architecture_files = [f for f in self._files if architecture in f["name"]]
        ...
        def file_filter(name):
            """
                ...
                a given name.
            """
            files = [f for f in architecture_files if name in f["name"]]
