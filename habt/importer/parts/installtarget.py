...
from habt.models import Architecture, Archive, Distribution, InstallTarget, Part
...
class InstallTargetImporter:
    ...
    def run(self, packages):
        ...
        for entry in packages:
            ...
            archive = Archive.get_or_add(url=source_list_entry.archive)
            distribution = Distribution.get_or_add(name=source_list_entry.distribution)

            for part in source_list_entry.parts:
                db_part = Part.get_or_add(name=part)
                ...
                InstallTarget.get_or_add(
                    ...
                    distribution=distribution,
                    part=db_part,
                    architecture=Architecture.get_or_add(name="all"),
                )
