...
from habt.models import (Architecture, Archive, Dependency, DependencySection, Distribution, InstallTarget, Package, PackageSection,
                         PackageVersion, Part)
...
class PackageImporter:
    ...
    def packages(self):
        ...
        for source in self._packages:
            ...
            for architecture in source["Architectures"]:
                ...
                installtarget = (
                    InstallTarget.query.join(Archive)
                    .join(Distribution)
                    .join(Part)
                    .join(Architecture)
                    .filter(
                ...
                for key, packages in groupby(
                    architecture["Packages"], lambda p: p["Package"]
                ):
                ...
                log.info(
                    "Added {0} debian Packages to the database".format(
                        Package.query.count()
                    )
                )

    def _package_version(
        self, package, version, source_packages, description_list, installtarget
    ):
        if package.id:
            db_version = PackageVersion.query.filter(
            ...
            if db_version:
                ...
                log.info(
                    "Version allready exists {0} ({1})".format(
        ...
        source_package = next(
            (s for s in source_packages if package.name in s["Binary"]),
            {},  # default value
        )
        ...
        description = next(
            (d for d in description_list if package.name in d["Package"]),
            {},  # default value
        )
    ...
    def _package_dependencies(self, version, package_version):
        for section in version.relations:
            dependency_section = DependencySection.query.filter(
            ...
            for relation in version.relations[section]:
                ...
                session.add(
                    Dependency(
                        ...
                        version="({0} {1})".format(
                            relation["version"][0], relation["version"][1]
                        )
                        if relation["version"]
                        else "",
                        package=package,
